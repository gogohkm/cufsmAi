...
def calc_beta_distortional(Lcrd: float, Lm: float,
    ...
    Lcrd : 왜곡좌굴 반파장 (in.)
    Lm : 비지지길이 (in.) — 랩 끝~변곡점
    M1 : 비지지구간 단부 모멘트 (부호 포함)
    M2 : 비지지구간 단부 모멘트 (부호 포함)

    M1, M2는 부호가 있는 단부 모멘트로 입력한다. 본 함수가 크기로 정렬해
    |M1| ≤ |M2|를 보장하므로 입력 순서는 무관하다. AISI S100-16 App. 2
    Eq. 2.3.3.3-3 규약상 M1/M2는 역곡률(reverse curvature)일 때 양(+),
    단곡률(single curvature)일 때 음(-)이다 (부호가 다른 단부 모멘트→음의 비).
    ...
    # 크기순 정렬: |M1| ≤ |M2| (Eq. 2.3.3.3-3에서 M1=작은 단부, M2=큰 단부)
    if abs(M1) > abs(M2):
        M1, M2 = M2, M1
    # M2는 큰 쪽 단부이므로 ≈0이면 두 단부 모두 ≈0 (모멘트 없음)
    if abs(M2) < 1e-10:
        return 1.0

    # 부호 있는 비율 (abs() 제거): 단곡률이면 음(-)→β 감소, 역곡률이면 양(+).
    # r ≥ -1로 제한하여 (1+r) ≥ 0 보장 (Eq. 2.3.3.3-3).
    r = max(M1 / M2, -1.0)
    beta = 1.0 + 0.4 * (L / Lm) ** 0.7 * (1.0 + r) ** 0.7
